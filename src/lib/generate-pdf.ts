...
/** Format number in Indian currency style: ₹1,25,000.00 */
function formatIndianCurrency(num: number, symbol: string): string {
  const fixed = num.toFixed(2);
  const [intPart, decPart] = fixed.split(".");
  const lastThree = intPart.slice(-3);
  const remaining = intPart.slice(0, -3);
  const formatted = remaining.replace(/\B(?=(\d{2})+(?!\d))/g, ",") + (remaining ? "," : "") + lastThree;
...
export async function generateQuotationPDF(data: QuotationData): Promise<void> {
  ...
  const tableData = data.items
    ...
    .map((item, index) => {
      return [
        ...
        formatIndianCurrency(item.unitPrice, data.currencySymbol),
        item.discount > 0 ? `${item.discount}%` : "—",
        `${item.gstRate}%`,
        formatIndianCurrency(item.total, data.currencySymbol),
      ];
    });
  ...
  drawTotalLine("Subtotal:", formatIndianCurrency(data.subtotal, data.currencySymbol), yPos + 10);
  drawTotalLine("Discount:", `-${formatIndianCurrency(data.totalDiscount, data.currencySymbol)}`, yPos + 20);
  drawTotalLine("GST:", formatIndianCurrency(data.totalGST, data.currencySymbol), yPos + 30);
  drawTotalLine("Freight:", formatIndianCurrency(data.freight, data.currencySymbol), yPos + 40);
  ...
  doc.setTextColor(...brandBlue);
  drawTotalLine("GRAND TOTAL:", formatIndianCurrency(data.grandTotal, data.currencySymbol), yPos + 55, true);
  ...
  doc.text("UpaHealth", pageWidth / 2, doc.internal.pageSize.getHeight() / 2, { align: "center", angle: 35 });
  ...
  doc.setDrawColor(...brandTeal);
  doc.setLineWidth(0.3);
  doc.roundedRect(margin, yPos, 60, 8, 2, 2, "FD");
  doc.setTextColor(...brandTeal);
  doc.setFontSize(6);
  doc.setFont("helvetica", "bold");
  doc.text("✦ Generated by UpaHealth AI", margin + 3, yPos + 5.5);
